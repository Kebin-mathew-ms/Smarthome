...
const VolunteerLoginPage = () => {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (!email || !password) {
      message.warning('Please enter your volunteer credentials.');
      return;
    }

    setLoading(true);
    try {
      const res = await volunteerService.login(email, password);
      if (res.success) {
        localStorage.setItem('token', res.data.token);
        localStorage.setItem('user', JSON.stringify(res.data.volunteer));
        message.success('Volunteer login successful!');
        window.location.href = ROUTES.VOLUNTEER_DASHBOARD;
      }
    } catch (err) {
    ...
    <div style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', background: '#f1f5f9', padding: 20 }}>
      <Card bordered={false} style={{ width: 400, borderRadius: 20, boxShadow: '0 10px 25px rgba(0,0,0,0.08)' }}>
        <div style={{ textAlign: 'center', marginBottom: 24 }}>
          ...
          <Title level={3} style={{ margin: 0 }}>Staff Volunteer Portal</Title>
          <Text type="secondary" style={{ fontSize: 13 }}>Sign in to access assigned tasks & community help</Text>
        </div>
        ...
        <form onSubmit={handleSubmit}>
          <div style={{ marginBottom: 16 }}>
            ...
            <Input
              ...
              placeholder="volunteer@home.com"
              size="large"
              value={email}
