...
const VolunteerDashboardPage = () => {
  ...
  const fetchDashboard = async () => {
    ...
    try {
      ...
      message.error(err.message || 'Failed to load volunteer dashboard');
    } finally {
      setLoading(false);
    }
  };
  ...
  if (loading || !data) {
    return (
      <div>
        <PageHeader title="Staff Volunteer Dashboard" subtitle="Loading..." />
        <SkeletonCard rows={6} />
      </div>
    );
  }
  ...
  return (
    <div>
      <PageHeader
        title={`Welcome, ${data.volunteer.volunteer_name}`}
        subtitle={`Designation: ${data.volunteer.designation} | Community Helper`}
      />
      ...
      <Title level={4}>Today's Assignments</Title>
      ...
      <Row gutter={[16, 16]}>
        {data.todayJobs.map(job => (
          <Col xs={24} sm={12} key={job.id}>
            <Card
              hoverable
              onClick={() => navigate(`/volunteer/bookings/${job.id}`)}
