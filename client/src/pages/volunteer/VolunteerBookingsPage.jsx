...
const VolunteerBookingsPage = () => {
  ...
  const fetchBookings = async () => {
    ...
    try {
      ...
      const res = await volunteerService.getAssignedBookings({ status: statusParam });
  ...
  const columns = [
    ...
    { title: 'Status', dataIndex: 'booking_status', key: 'booking_status', render: s => <StatusBadge status={s} /> },
    {
      ...
      key: 'action',
      render: (_, r) => (
        <Button type="primary" size="small" onClick={() => navigate(`/volunteer/bookings/${r.id}`)}>
          View Job
        </Button>
      )
    }
...
export default VolunteerBookingsPage;
