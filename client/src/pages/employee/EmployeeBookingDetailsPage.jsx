...
const EmployeeBookingDetailsPage = () => {
  const { id } = useParams();
  ...
  const fetchBooking = async () => {
    setLoading(true);
    try {
      const res = await employeeService.getAssignedBookingById(id);
      if (res.success) {
        setBooking(res.data);
        setCurrentStatus(res.data.booking_status);
      }
    } catch (err) {
      message.error(err.message || 'Failed to load job details');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleStatusChange = async (newStatus) => {
    ...
    try {
      await employeeService.updateBookingStatus(id, newStatus);
  ...
  const handleCheckOut = async () => {
    ...
    try {
      await employeeService.checkOut({ checkin_id: booking.checkin.id, notes: 'Completed work' });
  ...
  const handleSaveSignature = async () => {
    ...
    setSavingSig(true);
    try {
      await employeeService.saveCustomerSignature(id, dataUrl);
  ...
  if (loading || !booking) {
    return (
      <div>
        <PageHeader title="Field Assignment Details" subtitle="Loading..." />
        <SkeletonCard rows={8} />
      </div>
    );
  }
  ...
  const breadcrumbItems = [
    { title: 'My Assigned Jobs', path: ROUTES.EMPLOYEE_BOOKINGS },
  ...
  return (
    <div>
      ...
      <PageHeader
        title={`Field Job #${booking.booking_number}`}
      ...
      <Row gutter={[24, 24]}>
        <Col xs={24} lg={16}>
          ...
          <Card title="Technician Field Action Center" bordered={false} style={{ borderRadius: 16, marginBottom: 24 }}>
            <div style={{ display: 'flex', gap: 16, alignItems: 'center', marginBottom: 20, flexWrap: 'wrap' }}>
              <div>
                <label style={{ display: 'block', fontSize: 12, fontWeight: 600, marginBottom: 4 }}>Work Status Transitioner</label>
              ...
              <div>
                <label style={{ display: 'block', fontSize: 12, fontWeight: 600, marginBottom: 4 }}>GPS Check-In Tracker</label>
                {booking.checkin && !booking.checkin.check_out_time ? (
                  <Button type="primary" danger onClick={handleCheckOut}>
                    Check-Out from Job Site
                  </Button>
                ) : (
          ...
          <Card title="Customer & Dispatch Address Information" bordered={false} style={{ borderRadius: 16 }}>
...
export default EmployeeBookingDetailsPage;
